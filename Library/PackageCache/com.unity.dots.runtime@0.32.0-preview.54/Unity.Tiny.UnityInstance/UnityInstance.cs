...
namespace Unity.Tiny
{
    public unsafe class UnityInstance
    {
        ...
        unsafe delegate void BurstExceptionCallbackFn(byte* exceptionName, int exceptionNameLen, byte* errorMessage, int errorMessageLen);
        static readonly BurstExceptionCallbackFn s_RethrowBurstExceptionFn;
        static readonly IntPtr s_BurstExceptionCallbackFnPtr;
        unsafe static UnityInstance()
        {
        ...
        static unsafe void BurstExceptionCallback(byte* exceptionName, int exceptionNameLen, byte* errorMessage, int errorMessageLen)
        {
            var errorMessageString = System.Text.Encoding.UTF8.GetString(errorMessage, errorMessageLen);
#if !NET_DOTS
            var exceptionNameString = System.Text.Encoding.UTF8.GetString(exceptionName, exceptionNameLen);
            var exceptionType = Type.GetType(exceptionNameString, false, true);
            if (exceptionType != null)
            {
                var exception = (Exception) Activator.CreateInstance(exceptionType, errorMessageString);
                throw exception;
            }
#endif
        ...
        public static unsafe void BurstInit()
        {
        ...
        private static Entity FindSingleEntityWith(Type type, out EntityManager entityManager)
        {
                ...
                using (var query = w.EntityManager.CreateEntityQuery(type))
                {
                    ...
                    using (var entityArray = query.ToEntityArray(Allocator.TempJob))
                    {
                        if (entityArray.Length > 1)
                            throw new Exception($"More than one entity of {type} found");
